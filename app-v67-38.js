...
function showView(id){
  ...
  // V67.38 — o feed esportivo vive na página inicial.
...
/* ===== V67.38 — FEED ESPORTIVO DA 2ª CIA ===== */
...
function v6738RenderFeed(rows){
  ...
  box.innerHTML=rows.map(r=>{
    ...
    return `<article class="sports-feed-card" data-feed-id="${Number(r.id)}">
      <header class="feed-card-head"><div class="feed-avatar">${v6738FeedInitials(r.nome)}</div><div class="feed-author"><b>${v6738FeedEsc(r.graduacao||'BM')} ${v6738FeedEsc(r.nome||'Militar')}</b><span>${v6738FeedWhen(r.data_atividade)}</span></div><div class="feed-type-icon">${v6738CategoryIcon(r.categoria)}</div></header>
...
async function v6738LoadFeed(force=false){
  ...
  try{
    ...
    const rows=await v55RpcAll('feed_listar',{p_token_militar:cloudSession.token,p_limite:30});
    v6738FeedLastLoad=Date.now();v6738RenderFeed(rows);
  }catch(e){
    console.warn('Falha ao carregar feed:',e);
    box.innerHTML='<div class="sports-feed-state error">Não foi possível carregar o feed agora. Verifique se o SQL da v67.38 já foi instalado no Supabase.</div>';
