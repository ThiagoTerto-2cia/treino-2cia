...
function openExercise(id,inPlan=false){
  ...
  imgStart.src=hasRealistic?`assets/exercises/${realisticKey}-inicio.webp?v=27.0`:`assets/exercises/${currentExercise.id}-inicio.svg`;
  imgEnd.src=hasRealistic?`assets/exercises/${realisticKey}-fim.webp?v=27.0`:`assets/exercises/${currentExercise.id}-fim.svg`;
...
function exportHistory(){
  const blob=new Blob([JSON.stringify({exportedAt:new Date().toISOString(),version:'v27',history:history(),workouts:migrateWorkoutQuality()},null,2)],{type:'application/json'});
...
localStorage.setItem('t2_app_version','v27.0');
