...
async function officialTafAdminLoad(){
  ...
  try{
    ...
    const rowsPromise=v55RpcAll('admin_taf_oficial_listar_resultados',{p_token:adminSessionToken,p_edicao:initialCode});
    const [ed,initialRows]=await Promise.all([editionsPromise,rowsPromise]);
    ...
    if(sel){
      ...
      code=ed.some(e=>e.codigo===keep)?keep:(ed[0]?.codigo||initialCode);
      if(code&&!ed.some(e=>e.codigo===code))sel.insertAdjacentHTML('afterbegin',`<option value="${otEsc(code)}">TAF ${otEsc(code)}</option>`);
      sel.value=code;
    }
    if(!code){if(box)box.innerHTML='<div class="admin-empty">Nenhuma edição cadastrada.</div>';return}
    if(code===initialCode){officialTafAdminRender(initialRows);return}
    if(officialTafAdminCache.rows.has(code)){officialTafAdminRender(officialTafAdminCache.rows.get(code));return}
    ...
    officialTafAdminCache.rows.set(code,rows);officialTafAdminRender(rows);
  }catch(e){console.error(e);if(box)box.innerHTML='<div class="admin-empty">Não foi possível carregar o TAF oficial. Toque novamente em Administração para tentar de novo.</div>'}
}
...
try{localStorage.setItem('t2_app_version','v67.21')}catch(e){}
