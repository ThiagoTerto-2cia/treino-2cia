...
async function cloudInitialSync(){
  ...
  try{
    ...
    const merged=cloudMerge(local,remote);

    // Primeiro aplica tudo que existe na nuvem ao aparelho atual.
    cloudApply(merged);

    // Depois confirma o estado consolidado na nuvem.
    const saved=await cloudRpc('salvar_dados_matricula',{
    ...
    // Atualiza qualquer tela de dados que esteja aberta.
