...
function updateCardioRankingHint(){
  ...
  if(minutes>=30){
    ...
    hint.textContent=v66WorkoutServiceDay===true
      ? '✓ Critério de tempo atingido. Durante serviço validado, esta atividade pode gerar até +1 ponto de treino, respeitando o limite diário.'
...
function saveCardioWorkout(){
  ...
  const record={
    ...
    schemaVersion:6733,excludedFromStats:false,workoutType:'free',muscleGroups:['Cardio'],cardioMachine:machine,
  ...
  if(msg){
    ...
    msg.textContent=eligible
      ? '✓ Cardio salvo. Critério de 30 minutos atingido; poderá pontuar respeitando o limite diário do ranking.'
      : minutes>=30
...
function reliableWorkouts(){
  return migrateWorkoutQuality().filter(w=>!w.legacyData && !w.excludedFromStats && !isCardioRecord(w));
}
...
localStorage.setItem('t2_app_version','v67.34');
...
async function v55LoadRanking(){
 ...
 try{
   ...
   const explain=isAnnual
     ? '<b>O Ranking Anual soma os pontos dos rankings mensais do ano.</b> As regras não mudam: cada dia pode gerar no máximo 1 ponto de treino + 1 ponto de Core, somente quando a atividade cumpre os critérios e ocorre durante uma validação de serviço confirmada.'
     : '<b>Cada registro válido soma 1 ponto de frequência.</b> <b>Somente atividades realizadas durante uma validação de serviço ativa entram no ranking.</b> A validação exige o QR fixo da unidade e confirmação de localização no momento da leitura. Nos treinos prontos e personalizados, é necessário registrar pelo menos <b>3 exercícios e 6 séries</b>. No treino livre, é necessário informar grupo muscular e registrar pelo menos <b>20 minutos</b>. No <b>Cardio</b>, Esteira, Bicicleta Ergométrica e Simulador de Remo precisam ter pelo menos <b>30 minutos</b>; na Esteira também são registrados distância e calorias. No <b>Core Operacional</b>, é necessário concluir <b>uma rotina completa</b>. <b>Cardio entra na mesma categoria de treino para o ranking: no máximo 1 ponto de treino (pronto, personalizado, livre ou cardio) + 1 ponto de Core por militar, totalizando no máximo 2 pontos por dia.</b> Se houver outros treinos ou outras rotinas de Core no mesmo dia, eles podem permanecer no histórico, mas não geram pontos extras. Atividades sem validação de serviço ativa ou que não atinjam os critérios valem 0 ponto. Carga e peso levantado não aumentam a pontuação.';
   ...
   `<p class="v55-note">${isAnnual?'O total anual é a soma dos pontos mensais do ano, mantendo o teto diário de 1 treino + 1 Core.':'O ranking considera apenas registros válidos durante uma validação de serviço confirmada, com teto de 1 treino + 1 Core por dia.'} Cargas individuais não são exibidas.</p>`;
...
function renderServiceValidationCards(){
  ...
  for(const ids of targets){
    ...
    if(serviceActivityMode==='operational' && serviceValidationStatus.valid){
      ...
      if(hint)hint.textContent='Treino válido: até +1 ponto/dia. Core completo: até +1 ponto/dia.';
      continue;
    }
...
try{localStorage.setItem('t2_app_version','v67.34')}catch(e){}
