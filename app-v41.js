...
function cloudApi(path,options={}){
  const c=cloudCfg();
  return fetch(c.supabaseUrl.replace(/\/$/,'')+path,{
    ...options,
...
function cloudHideGate(){ document.body.classList.remove('auth-locked'); document.getElementById('authGate')?.classList.remove('show'); cloudRenderUser(); }
...
function cloudRenderUser(){
  ...
  document.getElementById('cloudUserGraduacao').textContent=cloudSession.graduacao||'';
...
async function cloudLogin(){
  ...
  try{
    ...
    cloudMsg('');
    cloudHideGate();
  }catch(e){console.error(e);cloudMsg('Não foi possível acessar. Verifique sua conexão.','error');}
}
...
async function cloudInitialSync(){
  ...
  try{
    ...
    cloudApply(merged);
    await cloudRpc('salvar_dados_militar',{p_token:cloudSession.token,p_dados:merged});
...
async function cloudPushNow(){
  ...
  try{
    if(!(await cloudValidate())){
      cloudSaveSession(null);cloudShowGate();cloudMsg('Matrícula inativa. Procure o administrador.','error');return;
    }
    await cloudRpc('salvar_dados_militar',{p_token:cloudSession.token,p_dados:cloudSnapshot()});
    const s=document.getElementById('cloudUserStatus');if(s)s.textContent='Sincronizado';
  }catch(e){const s=document.getElementById('cloudUserStatus');if(s)s.textContent='Pendente';}
}
...
async function cloudInit(){
  ...
  if(cloudSession?.token){
    if(!navigator.onLine){cloudHideGate();return;}
    if(await cloudValidate()){await cloudInitialSync();cloudHideGate();return;}
    cloudSaveSession(null);
  }
  cloudShowGate();
}
window.addEventListener('online',()=>{cloudRenderUser();if(cloudSession?.token)cloudInitialSync()});
