...
function openExercise(id,inPlan=false){
  ...
  imgStart.src=hasRealistic?`assets/exercises/${realisticKey}-inicio.webp?v=36.0`:`assets/exercises/${currentExercise.id}-inicio.svg`;
  imgEnd.src=hasRealistic?`assets/exercises/${realisticKey}-fim.webp?v=36.0`:`assets/exercises/${currentExercise.id}-fim.svg`;
...
function exportHistory(){
  const blob=new Blob([JSON.stringify({exportedAt:new Date().toISOString(),version:'v36',history:history(),workouts:migrateWorkoutQuality()},null,2)],{type:'application/json'});
...
localStorage.setItem('t2_app_version','v36.0');
...
function getWorkoutOfTheDay(){
  ...
  let history=[];
  try{
    history=JSON.parse(localStorage.getItem('t2_workouts')||'[]');
  }catch(e){ history=[]; }

  const official=history
    .filter(w=>{
      const p=(w.plan||w.planId||w.workout||'').toString().toUpperCase();
      const id=(p.match(/[ABCD]/)||[])[0];
      return id && !w.ignoreEvolution && !w.ignored && !w.test;
    })
    .sort((a,b)=>{
      const da=new Date(a.finishedAt||a.date||a.createdAt||0).getTime();
      const db=new Date(b.finishedAt||b.date||b.createdAt||0).getTime();
      return db-da;
    });

  let lastId=null;
  if(official.length){
    const p=(official[0].plan||official[0].planId||official[0].workout||'').toString().toUpperCase();
    lastId=(p.match(/[ABCD]/)||[])[0]||null;
  }else{
    try{
      const last=JSON.parse(localStorage.getItem('t2_last')||'null');
      if(last){
        const p=(last.plan||last.planId||last.workout||'').toString().toUpperCase();
        lastId=(p.match(/[ABCD]/)||[])[0]||null;
      }
    }catch(e){}
  }

  const nextId = lastId ? order[(order.indexOf(lastId)+1)%order.length] : 'A';
  const names={
    A:'Peito + Tríceps',
    B:'Costas + Bíceps',
    C:'Pernas',
  ...
  return {
    id: nextId,
    title:`Treino ${nextId}`,
    subtitle:names[nextId],
    last:lastId
  };
}

function renderWorkoutOfTheDay(){
  const card=document.getElementById('workoutOfDayCard');
  if(!card) return;
  const w=getWorkoutOfTheDay();

  const title=card.querySelector('[data-wod-title]');
  const subtitle=card.querySelector('[data-wod-subtitle]');
  const meta=card.querySelector('[data-wod-meta]');
  const btn=card.querySelector('[data-wod-start]');

  if(title) title.textContent=w.title;
  if(subtitle) subtitle.textContent=w.subtitle;
  if(meta){
    meta.textContent=w.last
      ? `Último treino: ${w.last} • Próximo recomendado`
      : 'Sem treino anterior • Comece pelo Treino A';
  }
  if(btn){
    btn.dataset.plan=w.id;
    btn.onclick=()=>{
      // Localiza a chave REAL em DATA.plans, por exemplo:
      // "Treino A — Peito + Tríceps", em vez de tentar abrir apenas "A".
      const planKey=Object.keys(DATA.plans||{}).find(name=>{
        const n=normText(name);
        return n.startsWith(`treino ${w.id.toLowerCase()}`) ||
               new RegExp(`(^|\\s)${w.id.toLowerCase()}(\\s|$)`).test(n);
      });

      if(!planKey || !DATA.plans[planKey]){
        alert('Não foi possível localizar o treino recomendado.');
        return;
      }

      activePlanName=planKey;
      activePlanExercises=DATA.plans[planKey].map(exByName).filter(Boolean);
      activePlanIndex=-1;

      if(!activePlanExercises.length){
        alert('Não foi possível carregar os exercícios deste treino.');
        return;
      }

      // "INICIAR" agora inicia de verdade o treino do dia,
      // abrindo diretamente o primeiro exercício.
      startPlanWorkout();
    };
  }
}
...
document.addEventListener('DOMContentLoaded',()=>{
  setTimeout(renderWorkoutOfTheDay,120);
});
window.addEventListener('storage',()=>renderWorkoutOfTheDay());
