...
function cardioBindInputs(){
  ['cardioMinutes','cardioDistance','cardioCalories'].forEach(id=>{
    const el=byId(id); if(el)el.oninput=()=>{updateCardioRankingHint();saveNavigationState('cardioWorkout')};
  });
}
...
function openWorkoutHistory(index){
  ...
  if(isCardioRecord(w)){
    ...
    const extra=w.cardioMachine==='treadmill'
      ? `<div><strong>${Number(w.cardioDistanceKm||0).toLocaleString('pt-BR',{maximumFractionDigits:2})}</strong><span>km</span></div><div><strong>${Math.round(Number(w.cardioCalories||0))}</strong><span>kcal</span></div>`
      : '';
...
localStorage.setItem('t2_app_version','v67.33');
...
try{localStorage.setItem('t2_app_version','v67.33')}catch(e){}
