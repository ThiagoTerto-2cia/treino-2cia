...
async function v67382DeleteWorkoutHistory(index){
  ...
  // v67.38.6: primeiro confirma a remoção no servidor. Só depois apaga localmente.
  ...
  try{
    ...
    if(!feedOk){
      ...
      // Se o post já não existe no servidor, pode prosseguir com a exclusão local.
      feedOk=true;
    }
  }catch(e){
...
async function v67383DeleteFeedDescriptor(desc,queueOnFail=true){
  ...
  try{
    // V67.38.6: uma única RPC faz a exclusão por atividade_id, JSON interno,
...
async function v67384ReconcileMyFeed(){
  ...
  try{
    // V67.38.6: se o Histórico estiver vazio, o Feed próprio também deve ficar vazio.
...
function v6738CompactActivity(w){
  ...
  // V67.38.6: a localização vira parte da própria atividade no momento em que ela
...
async function v67382RefreshFeed(button){
  ...
  try{
    v6738FeedLastLoad=0;
    // V67.38.6: o botão atualizar também reconcilia as publicações deste militar
