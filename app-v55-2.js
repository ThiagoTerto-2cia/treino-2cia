...
/* =========================
   V55.2 — PERSISTÊNCIA PERFIL / RANKING
   ========================= */
function v552RestoreSpecialView(){
  try{
    if(!cloudSession?.matricula) return;
    const raw=localStorage.getItem(NAV_STATE_KEY);
    if(!raw) return;
    const st=JSON.parse(raw);
    if(!st) return;
    if(cloudNormalize(st.matricula)!==cloudNormalize(cloudSession.matricula)) return;

    const wanted=st.view || st.viewId || st.activeView || '';
    if(wanted==='v55Profile'){
      v55RenderProfile();
      showView('v55Profile');
      if(Number.isFinite(Number(st.scrollY))){
        setTimeout(()=>window.scrollTo(0,Number(st.scrollY)||0),50);
      }
    }else if(wanted==='v55Ranking'){
      showView('v55Ranking');
      v55LoadRanking();
      if(Number.isFinite(Number(st.scrollY))){
        setTimeout(()=>window.scrollTo(0,Number(st.scrollY)||0),120);
      }
    }
  }catch(e){
    console.warn('V55.2: não foi possível restaurar a tela especial',e);
  }
}

// O cloudInit pode terminar depois do carregamento inicial.
// Fazemos duas tentativas leves para recolocar o usuário na tela onde estava.
window.addEventListener('load',()=>{
  setTimeout(v552RestoreSpecialView,500);
  setTimeout(v552RestoreSpecialView,1400);
});
