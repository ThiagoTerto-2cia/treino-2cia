...
function showView(id){
  ...
  if(!navRestoring && !browserNavHandling && previous!==id){
    try{
      let trail=JSON.parse(sessionStorage.getItem('t2_view_trail_v663')||'[]');
      if(!Array.isArray(trail))trail=[];
      if(!trail.length && previous)trail.push(previous);
      if(trail[trail.length-1]!==id)trail.push(id);
      sessionStorage.setItem('t2_view_trail_v663',JSON.stringify(trail.slice(-30)));
    }catch(e){}
  }

  // Cada mudança de tela interna cria uma entrada no histórico do navegador.
  // Assim, o botão Voltar do Android retorna à tela anterior do app.
  if(!navRestoring && !browserNavHandling && previous!==id){
    try{history.pushState({t2View:id},'',location.href)}catch(e){}
  }
}
window.addEventListener('popstate',()=>{
  ...
  try{
    const state=JSON.parse(localStorage.getItem(NAV_STATE_KEY)||'null');
    const current=document.querySelector('.view.active')?.id||'home';
    // Use our own lightweight view trail saved in sessionStorage.
    let trail=[];
    try{trail=JSON.parse(sessionStorage.getItem('t2_view_trail_v663')||'[]')}catch(e){}
    if(trail.length){
      // current is last entry; discard it and return to prior.
      if(trail[trail.length-1]===current)trail.pop();
      const prev=trail.pop()||'home';
      sessionStorage.setItem('t2_view_trail_v663',JSON.stringify([...trail,prev]));
      restoreViewV663(prev);
    }else{
      restoreViewV663('home');
    }
    // Keep the PWA/site inside its own history envelope.
    try{history.pushState({t2View:document.querySelector('.view.active')?.id||'home'},'',location.href)}catch(e){}
  }finally{
    browserNavHandling=false;
  }
});
function restoreViewV663(id){
  switch(id){
    case 'history': showView('history');renderHistory();break;
    case 'progress': showView('progress');renderProgress();break;
    case 'stretch': showView('stretch');renderStretch();break;
    case 'tools': showView('tools');break;
    case 'taf': openTAF();break;
    case 'v55Profile': openV55Profile();break;
    case 'v55Ranking': showView('v55Ranking');v55LoadRanking();break;
    case 'workoutChooser': showView('workoutChooser');updateServiceChoiceUI();break;
    case 'freeWorkout': showView('freeWorkout');break;
    case 'plans': openPlans();break;
    case 'groups': openGroups();break;
    default: showView('home');break;
  }
}
...
function openFreeWorkout(){
  ...
  if(box)box.innerHTML=V66_FREE_GROUPS.map(g=>`<button type="button" class="free-muscle" data-group="${g}" onclick="this.classList.toggle('selected')">${g}</button>`).join('');
  const mins=byId('freeWorkoutMinutes');if(mins)mins.value='';
...
function saveFreeWorkout(){
  ...
  showSavedConfirmation(()=>openWorkoutChooser());
}
...
localStorage.setItem('t2_app_version','v66.3');
...
async function v55LoadRanking(){
 ...
 <div class="card v55-rank-explain"><h3>Como funciona a pontuação?</h3><p><b>Cada treino válido soma 1 ponto de frequência.</b> <b>Somente treinos realizados em dia de serviço entram no ranking.</b> Nos treinos prontos e personalizados, é necessário registrar pelo menos <b>3 exercícios e 6 séries</b>. No treino livre, é necessário informar grupo muscular e registrar pelo menos <b>20 minutos</b> de treino. Treinos realizados fora do serviço ou que não atingirem esses critérios continuam salvos normalmente, mas valem 0 ponto no ranking. Carga e peso levantado não aumentam a pontuação.</p></div>`+
 ...
 '<p class="v55-note">O ranking de frequência considera somente treinos válidos realizados em dias de serviço. Cargas individuais não são exibidas.</p>';
...
// V66.3 — cria uma entrada interna para o primeiro uso do botão Voltar.
window.addEventListener('load',()=>{
  try{
    if(!history.state?.t2View){
      history.replaceState({t2View:document.querySelector('.view.active')?.id||'home'},'',location.href);
      history.pushState({t2View:document.querySelector('.view.active')?.id||'home'},'',location.href);
    }
  }catch(e){}
});
