...
async function validateServiceQr(qr){
  const st=byId('serviceQrScanStatus');if(st)st.textContent='QR lido. Confirmando localização…';
  try{
    ...
    const row=await cloudRpc('validar_presenca_servico',{
      p_token_militar:cloudSession.token,p_qr_token:String(qr||'').trim(),p_lat:pos.coords.latitude,p_lon:pos.coords.longitude,p_accuracy:pos.coords.accuracy,p_device_id:serviceDeviceId()
    });
...
try{localStorage.setItem('t2_app_version','v67.19')}catch(e){}

/* ===== V67.20 — RANKING OFICIAL TAF ===== */
let officialTafState={edition:null,editions:[],rows:[],scope:'all',view:'general',exercise:'pushup'};
...
function otValue(r,k){if(k==='run'||k==='swim')return otSec(r[k]);const n=Number(r[k]);return Number.isFinite(n)&&r[k]!==null&&r[k]!==''?n:null}
...
function otGeneral(rows){
 const keys=['bar','pushup','abs','run'];
 const complete=rows.filter(r=>keys.every(k=>otEligible(r,k)));
 const ranks={}; keys.forEach(k=>{ranks[k]=new Map(otCompetitionRanks(complete,k).map(x=>[String(x.r.id),x.rank]))});
 return complete.map(r=>({r,total:keys.reduce((s,k)=>s+(ranks[k].get(String(r.id))||999),0),places:keys.map(k=>ranks[k].get(String(r.id)))}))
 .sort((a,b)=>a.total-b.total||a.places.filter(x=>x===1).length*-1-b.places.filter(x=>x===1).length*-1||String(a.r.nome).localeCompare(String(b.r.nome),'pt-BR'));
}
function otMedal(i){return i===0?'🥇':i===1?'🥈':i===2?'🥉':`${i+1}º`}
...
function otViewTabs(){return `<div class="ot-seg ot-view"><button class="${officialTafState.view==='general'?'active':''}" onclick="officialTafSetView('general')">CLASSIFICAÇÃO GERAL</button><button class="${officialTafState.view==='exercise'?'active':''}" onclick="officialTafSetView('exercise')">POR EXERCÍCIO</button></div>`}
...
function officialTafSetScope(v){officialTafState.scope=v;renderOfficialTafRanking()}
function officialTafSetView(v){officialTafState.view=v;renderOfficialTafRanking()}
function officialTafSetExercise(v){officialTafState.exercise=v;renderOfficialTafRanking()}
function officialTafSetEdition(v){officialTafState.edition=v;officialTafLoadRows()}
async function openOfficialTafRanking(){showView('officialTafRanking');const box=byId('officialTafRankingContent');if(box)box.innerHTML='<div class="card"><p>Carregando ranking oficial…</p></div>';try{const ed=await v55RpcAll('taf_oficial_listar_edicoes',{p_token_militar:cloudSession.token});officialTafState.editions=ed;officialTafState.edition=officialTafState.edition||ed[0]?.codigo||null;await officialTafLoadRows()}catch(e){console.error(e);if(box)box.innerHTML='<div class="card"><p>Não foi possível carregar o Ranking Oficial TAF. Instale o SQL da V67.20.</p></div>'}}
async function officialTafLoadRows(){if(!officialTafState.edition)return renderOfficialTafRanking();try{officialTafState.rows=await v55RpcAll('taf_oficial_listar_resultados',{p_token_militar:cloudSession.token,p_edicao:officialTafState.edition});renderOfficialTafRanking()}catch(e){console.error(e)}}
function renderOfficialTafRanking(){
 ...
 if(officialTafState.view==='general'){
   const ranked=otGeneral(rows);content=ranked.length?ranked.map((x,i)=>`<div class="card ot-rank"><div class="ot-pos">${otMedal(i)}</div><div class="ot-person"><b>${otEsc(x.r.nome)}</b><span>${otEsc(x.r.graduacao||'BM')} • ${x.r.faixa_etaria==='38+'?'38+':'Até 37'}</span><small>Barras ${otValueLabel(x.r,'bar')} • Flexões ${otValueLabel(x.r,'pushup')} • Abd. ${otValueLabel(x.r,'abs')} • Corrida ${otValueLabel(x.r,'run')}</small></div></div>`).join(''):`<div class="card"><p>Nenhum militar possui os quatro índices necessários para a classificação geral nesta seleção.</p></div>`;
 }else{
   const k=officialTafState.exercise;const ranked=otCompetitionRanks(rows,k);content=otExerciseTabs()+(ranked.length?ranked.map((x,i)=>`<div class="card ot-rank"><div class="ot-pos">${x.rank<=3?['🥇','🥈','🥉'][x.rank-1]:x.rank+'º'}</div><div class="ot-person"><b>${otEsc(x.r.nome)}</b><span>${otEsc(x.r.graduacao||'BM')} • ${x.r.faixa_etaria==='38+'?'38+':'Até 37'}</span></div><div class="ot-index"><b>${otValueLabel(x.r,k)}</b><span>${k==='run'||k==='swim'?'tempo':'repetições'}</span></div></div>`).join(''):`<div class="card"><p>Nenhum índice registrado para este exercício.</p></div>`);
 }
 box.innerHTML=`<div class="ot-hero"><span class="eyebrow">2ª CIA / 4º BBM • RESULTADO OFICIAL</span><h2>🏆 Ranking Oficial TAF</h2><p>Comparativo motivacional pelos índices oficiais da tropa. Sem pontuação artificial.</p><label>Edição<select onchange="officialTafSetEdition(this.value)">${editionSelect}</select></label></div>${otViewTabs()}${otScopeTabs()}<div class="card ot-rule"><b>${officialTafState.view==='general'?'Como é definida a classificação geral?':'Como é definida a classificação por exercício?'}</b><p>${officialTafState.view==='general'?'São consideradas Barras, Flexões, Abdominais e Corrida. A ordem usa a soma das colocações nesses quatro índices; quanto menor, melhor. A Natação não participa do ranking geral. Só entra no geral quem possui os quatro índices.':'Mais repetições vencem em Barras, Flexões e Abdominais. Menor tempo vence em Corrida e Natação. Campo sem índice não é tratado como zero.'}</p></div>${content}<p class="v55-note">${ed?.observacao?otEsc(ed.observacao):'Dados oficiais alimentados pela Administração da unidade.'}</p>`;
}

/* Administração do TAF oficial */
async function officialTafAdminLoad(){if(!adminSessionToken)return;const sel=byId('officialTafAdminEdition'),box=byId('officialTafAdminList');if(box)box.innerHTML='<div class="admin-loading">Carregando TAF oficial…</div>';try{const ed=await v55RpcAll('admin_taf_oficial_listar_edicoes',{p_token:adminSessionToken});if(sel){const keep=sel.value;sel.innerHTML=ed.map(e=>`<option value="${otEsc(e.codigo)}">${otEsc(e.titulo||e.codigo)}</option>`).join('');if(ed.some(e=>e.codigo===keep))sel.value=keep}const code=sel?.value||ed[0]?.codigo;if(!code){if(box)box.innerHTML='<div class="admin-empty">Nenhuma edição cadastrada.</div>';return}const rows=await v55RpcAll('admin_taf_oficial_listar_resultados',{p_token:adminSessionToken,p_edicao:code});officialTafAdminRender(rows)}catch(e){console.error(e);if(box)box.innerHTML='<div class="admin-empty">Não foi possível carregar. Instale o SQL da V67.20.</div>'}}
function officialTafAdminRender(rows){const box=byId('officialTafAdminList');if(!box)return;box.innerHTML=rows.map(r=>`<div class="ot-admin-row" data-id="${r.id}"><div class="ot-admin-name"><b>${otEsc(r.nome)}</b><small>${otEsc(r.graduacao||'BM')}</small></div><div class="ot-admin-grid"><label>Faixa<select class="ot-age"><option value="ATE37" ${r.faixa_etaria==='ATE37'?'selected':''}>Até 37</option><option value="38+" ${r.faixa_etaria==='38+'?'selected':''}>38+</option></select></label><label>Barras<input class="ot-bar" type="number" min="0" value="${r.bar??''}"></label><label>Flexões<input class="ot-push" type="number" min="0" value="${r.pushup??''}"></label><label>Abd.<input class="ot-abs" type="number" min="0" value="${r.abs??''}"></label><label>Corrida<input class="ot-run" inputmode="numeric" placeholder="6:55" value="${r.run_seconds!=null?otTime(r.run_seconds):''}"></label><label>Natação<input class="ot-swim" inputmode="numeric" placeholder="1:29" value="${r.swim_seconds!=null?otTime(r.swim_seconds):''}"></label></div><div class="ot-admin-actions"><label><input class="ot-part" type="checkbox" ${r.participou!==false?'checked':''}> Participou</label><button onclick="officialTafAdminSave(${r.id})">SALVAR</button></div></div>`).join('')}
async function officialTafAdminSave(id){const row=document.querySelector(`.ot-admin-row[data-id="${id}"]`);if(!row)return;const val=(q)=>row.querySelector(q)?.value||null;try{await cloudRpc('admin_taf_oficial_salvar_resultado',{p_token:adminSessionToken,p_id:id,p_faixa_etaria:val('.ot-age'),p_bar:val('.ot-bar')===''?null:Number(val('.ot-bar')),p_pushup:val('.ot-push')===''?null:Number(val('.ot-push')),p_abs:val('.ot-abs')===''?null:Number(val('.ot-abs')),p_run_seconds:otSec(val('.ot-run')),p_swim_seconds:otSec(val('.ot-swim')),p_participou:row.querySelector('.ot-part').checked});adminMsg('Índice oficial salvo.','success')}catch(e){console.error(e);adminMsg('Não foi possível salvar o índice.','error')}}
async function officialTafAdminNewEdition(){const code=prompt('Código da nova edição (ex.: 2026.2):');if(!code)return;const title=prompt('Título da edição:',`TAF ${code}`)||`TAF ${code}`;try{await cloudRpc('admin_taf_oficial_criar_edicao',{p_token:adminSessionToken,p_codigo:code.trim(),p_titulo:title.trim()});await officialTafAdminLoad();adminMsg('Nova edição criada com a tropa cadastrada.','success')}catch(e){console.error(e);adminMsg('Não foi possível criar a edição.','error')}}
const _v6720OpenAdmin=openAdmin;openAdmin=function(){_v6720OpenAdmin();if(adminSessionToken)setTimeout(officialTafAdminLoad,80)};

try{localStorage.setItem('t2_app_version','v67.20')}catch(e){}
