...
function openExercise(id,inPlan=false){
  ...
  const realisticIds=new Set([1]);
  const hasRealistic=realisticIds.has(Number(currentExercise.id));
  ...
  imgStart.src=hasRealistic?`assets/exercises/${currentExercise.id}-inicio.webp?v=19.0`:`assets/exercises/${currentExercise.id}-inicio.svg`;
  imgEnd.src=hasRealistic?`assets/exercises/${currentExercise.id}-fim.webp?v=19.0`:`assets/exercises/${currentExercise.id}-fim.svg`;
...
function exportHistory(){
  const blob=new Blob([JSON.stringify({exportedAt:new Date().toISOString(),version:'v19',history:history(),workouts:migrateWorkoutQuality()},null,2)],{type:'application/json'});
...
localStorage.setItem('t2_app_version','v19.0');
