...
function saveFreeWorkout(){
  ...
  // Confirmação permanece visível NA PRÓPRIA TELA do treino livre.
  const savedMsg=byId('freeWorkoutSavedMessage');
  if(savedMsg){
  ...
  // Limpa os campos para deixar claro que o registro foi concluído,
  // sem tirar o militar da tela antes que ele veja a confirmação.
  document.querySelectorAll('#freeMuscleGroups .free-muscle.selected')
    .forEach(el=>el.classList.remove('selected'));
  const mins=byId('freeWorkoutMinutes');
  if(mins) mins.value='';

  if(typeof showWorkoutSavedMessage==='function') showWorkoutSavedMessage();
}
...
localStorage.setItem('t2_app_version','v66.5');
