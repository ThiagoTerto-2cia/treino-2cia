...
function renderCoreOperational(){
  ...
  box.innerHTML=`
    <div class="core-op-hero">
      ...
      <p>Rotinas guiadas para estabilizar o tronco durante transporte de carga, tração, arrasto, deslocamento baixo e trabalho com EPI.</p>
    ...
    <div class="core-op-grid">
      ${Object.entries(CORE_OPERATIONAL_ROUTINES).map(([key,r])=>`
        <button class="core-op-card" onclick="openCoreRoutine('${key}')">
          ...
          <span class="core-op-time">${r.duration}</span>
        </button>`).join('')}
    </div>
    <div class="core-op-note"><b>Prioridade: controle.</b> Aumente carga somente quando conseguir manter tronco e pelve estáveis. Dor aguda, formigamento, tontura ou perda de controle são sinais para interromper.</div>`;
    ...
    <div class="mobility-list">
      ${r.steps.map((s,i)=>`
        <div class="mobility-list-row ${i===coreStep?'active':''} ${i<coreStep?'done':''}">
          ...
          <div><b>${s.name}</b><small>${coreFmt(s.time)}</small></div>
        </div>`).join('')}
    </div>`;
}
function coreToggle(){
...
localStorage.setItem('t2_app_version','v67.1');
