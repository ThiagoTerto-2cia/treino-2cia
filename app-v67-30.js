...
function serviceCaptureDirectEntry(){
  try{
    ...
    if((mode==='servico'||mode==='treino'||mode==='quartel')&&qr){
...
function serviceClearDirectEntry(){try{sessionStorage.removeItem(SERVICE_DIRECT_KEY)}catch(e){}}
...
async function serviceDirectValidate(){
  ...
  try{
    ...
    // V67.30 — após validar pelo QR, encerra definitivamente o fluxo de entrada direta.
    // Isso impede que um refresh reabra a tela de validação e direciona sempre à Home.
    serviceClearDirectEntry();
    serviceHideDirectChoice();
    byId('serviceDirectLoginHint')?.remove();
    try{
      ...
      u.searchParams.delete('token');
      const clean=u.pathname+(u.search?u.search:'')+(u.hash||'');
      window.history.replaceState({t2App:true,snapshot:appHistorySnapshot('home')},'',clean);
...
try{localStorage.setItem('t2_app_version','v67.25')}catch(e){}
