...
async function v67382DeleteWorkoutHistory(index){
  ...
  await v67383DeleteFeedActivity(w);
...
/* ===== V67.38.4 — FEED ESPORTIVO DA 2ª CIA ===== */
...
async function v67383DeleteFeedDescriptor(desc,queueOnFail=true){
  ...
  try{
    // V67.38.4: uma única RPC faz a exclusão por atividade_id, JSON interno,
    // source_key ou data+plano. Só consideramos sucesso quando o banco confirma
    // que pelo menos uma publicação foi realmente removida.
    const r=await cloudRpc('feed_excluir_atividade_v67384',{
    ...
    const ok=r?.excluido===true||Number(r?.quantidade)>0;
    if(ok)return true;
...
async function v67384ReconcileMyFeed(){
  ...
  // Lista vazia não é usada aqui para evitar apagar o Feed antes da restauração
  // da nuvem. A limpeza total já é tratada por feed_excluir_minhas_atividades.
...
function v6738CompactActivity(w){
  ...
  // V67.38.4: a localização vira parte da própria atividade no momento em que ela
  // é concluída. Mudar a preferência depois não altera publicações anteriores.
  const pref=v6738FeedPrefs();
  const activityLocation=(pref.enabled&&pref.text)?pref.text:null;
  return {
...
async function v6738FlushFeedPending(){
  ...
  for(const item of items){
    try{
      ...
      // O fallback abaixo atende apenas pendências antigas criadas antes da v67.38.4.
...
async function v67382RefreshFeed(button){
  ...
  try{
    v6738FeedLastLoad=0;
    // V67.38.4: o botão atualizar também reconcilia as publicações deste militar
    // com o Histórico local/nuvem, removendo posts órfãos de atividades já excluídas.
