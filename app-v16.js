  ...
  if(prBox) prBox.innerHTML=prs.length?prs.slice(0,12).map(p=>{
    ...
    return `<div class="pr-card ${increased?'pr-new':'pr-steady'}">
      <div class="pr-title-row">
        ...
        <div class="pr-title"><b>${p.exercise}</b><small>${new Date(p.date).toLocaleDateString('pt-BR')}</small></div>
      </div>
...
function renderExerciseProgress(name){
  ...
  const summary=entries.length>1
    ...
    : `<div class="exercise-summary one"><div><span>Primeiro registro</span><b>${last} kg</b></div></div>`;
...
function exportHistory(){
  const blob=new Blob([JSON.stringify({exportedAt:new Date().toISOString(),version:'v16',history:history(),workouts:migrateWorkoutQuality()},null,2)],{type:'application/json'});
...
localStorage.setItem('t2_app_version','v16.0');
