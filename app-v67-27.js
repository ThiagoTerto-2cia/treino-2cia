...
window.addEventListener('popstate',(event)=>{
  ...
  try{
    ...
    if(active!=='home'){
      ...
      localStorage.setItem(NAV_STATE_KEY,JSON.stringify({...snap,matricula:cloudSession.matricula}));
      restoreNavigationState();
    }
  }catch(e){
...
function serviceShowDirectChoice(){
  ...
  if(!cloudSession?.token){
    const gate=document.querySelector('#authGate .auth-card')||byId('authGate');
