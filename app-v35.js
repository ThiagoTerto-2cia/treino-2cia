...
function openExercise(id,inPlan=false){
  ...
  imgStart.src=hasRealistic?`assets/exercises/${realisticKey}-inicio.webp?v=35.0`:`assets/exercises/${currentExercise.id}-inicio.svg`;
  imgEnd.src=hasRealistic?`assets/exercises/${realisticKey}-fim.webp?v=35.0`:`assets/exercises/${currentExercise.id}-fim.svg`;
...
function exportHistory(){
  const blob=new Blob([JSON.stringify({exportedAt:new Date().toISOString(),version:'v35',history:history(),workouts:migrateWorkoutQuality()},null,2)],{type:'application/json'});
...
localStorage.setItem('t2_app_version','v35.0');
...
function renderWorkoutOfTheDay(){
  ...
  if(btn){
    ...
    btn.onclick=()=>{
      const planId=w.id;
      const possibleFns=[
        'openWorkoutPlan','openPlan','startPlan','openWorkout','selectWorkoutPlan'
      ];
      for(const fn of possibleFns){
        if(typeof window[fn]==='function'){
          try{ window[fn](planId); return; }catch(e){}
        }
      }

      // Fallback: save selected plan and open workouts screen.
      localStorage.setItem('t2_active_plan', planId);
      const nav=[...document.querySelectorAll('button,a,[data-view],[data-screen]')].find(el=>{
        const txt=(el.textContent||'').trim().toLowerCase();
        return txt==='treinos' || txt.includes('treinos');
      });
      if(nav) nav.click();
    };
  }
