...
async function cloudInit(){
  ...
  if(cloudSession?.token){
    ...
    const needsFreshRestore=owner!==currentMat;
    if(needsFreshRestore){cloudClearTrackedLocal();cloudSetOwner(currentMat);}
    if(!navigator.onLine){cloudHideGate();return;}
    const validity=await cloudValidate();
    if(validity===true){await cloudInitialSync(needsFreshRestore);cloudHideGate();return;}
    if(validity===null){cloudHideGate();const status=document.getElementById('cloudUserStatus');if(status)status.textContent='Pendente';return;}
    if(validity===false){
      cloudClearTrackedLocal();cloudSetOwner('');cloudSaveSession(null);cloudShowGate();
      cloudMsg('Matrícula inativa ou acesso encerrado.','error');return;
    }
  }
  cloudShowGate();
}
