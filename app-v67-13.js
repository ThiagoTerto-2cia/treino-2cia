...
async function v55LoadRanking(){
 ...
 '<p class="v55-note">O ranking considera apenas registros válidos durante uma validação de serviço confirmada, com teto de 1 treino + 1 Core por dia. Cargas individuais não são exibidas.</p>';
...
openAdmin=function(){_v6711OpenAdmin();if(adminSessionToken)setTimeout(adminLoadServiceConfig,50)};
const _v6711AdminLoadMilitares=adminLoadMilitares;
adminLoadMilitares=async function(){const r=await _v6711AdminLoadMilitares();if(adminSessionToken)adminLoadServiceConfig();return r};

window.addEventListener('online',()=>refreshServiceValidationStatus());
try{localStorage.setItem('t2_app_version','v67.13')}catch(e){}
